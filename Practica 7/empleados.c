...
int verificarEmpleado(struct Empleado *empleados, int i, const char *nombre) {
    if (i == -1) {
        printf("%s no existe\n", nombre);
...
void editarEmpleado(struct Empleado empleados[MAX_EMPLEADO]) {
    ...
    if (i != -1) {  
        ...
        actualizarEmpleado(empleados, i, nombre, sexo, sueldo, puesto);  
    } else {
...
void imprimirEmpleadosConSueldoExtremos(struct Empleado *empleados) {
    ...
    printf("Empleado(s) con el sueldo mayor de %.2f:\n", mayorSueldo);
    mostrarEmpleadosConSueldo(empleados, mayorSueldo);

    printf("Empleado(s) con el sueldo menor de %.2f:\n", menorSueldo);
...
void ejecutarPrograma(){
    ...
    do {
        ...
        printf("6. Ver empleados con sueldo maximo y minimo\n");
        printf("7. Buscar empleado\n");
        printf("8. Salir\n");
